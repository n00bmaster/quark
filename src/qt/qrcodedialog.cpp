...
QString QRCodeDialog::getURI()
{
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non BTL unit input in lnReqAmount, we generate the URI with BTL as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::BTL, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
